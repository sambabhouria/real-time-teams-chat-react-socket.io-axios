 ...
 */
const SOCKET_URI = 'http://localhost:8002' //process.env.REACT_APP_SERVER_URI;
/**
 * App Component
 *
...
class App extends Component {
  ...
  state = {
    ...
    users: [{id: 1, name: "Alexa"}], // Avaiable users for signing-in
  ...
  componentDidMount() {
    ...
    console.log("in teh compoenend did mount",  fetchUsers ().then(
      (res) => {
       console.log('resultat appell', res);
       this.setState({
         users: res,
  ...
  initAxios() {
    ...
    axios.interceptors.response.use(
      response => {
        console.log('in the response', response.data.data)
        this.setState({ loading: false });
        return response;
      },
      error => {
